...
; For NAS, value is $FF.
; ---------------------------------------
                    lda       $C7C2               ;value will be $FF or $00 (MIL delay)
                    bne       .LCA6F              ;branch if not zero

                    lda       port1data
                    ora       #$01                ;EFI warning light OFF
                    sta       port1data
                    #ifdef    OBSOLETE_CODE
...
.LCA7C              bita      #$80
                    ...
                    ora       #$80                ;set milTestDelay bit 17 (a signal to code below)
                    sta       milTestDelay

; fault masks used here: $77,$BD,$00,$D0,$20,$00
; (see faultMasks in faults.asm)
.checkFaultBits     lda       faultBits_49
...
.efiLampOn          lda       port1data           ;if here, fail bit was found
                    anda      #$FE                ;EFI warning light ON
                    sta       port1data
                    lda       milTestDelay
                    ora       #$80                ;set milTestDelay bit 15 (a signal to code below))
                    sta       milTestDelay
                    #ifdef    OBSOLETE_CODE
                    jmp       Write9x00
...
.efiLampOff         lda       port1data           ;if here, no fault was found
                    ora       #$01                ;EFI warning light OFF
                    sta       port1data
; ----------------------------
...
.LCA6F              lda       faultBits_4D
                    anda      #$EF                ;mask out bit 4 (DTC 59 -- Group Fault)
                    ora       faultBits_4B        ;OR in all bits from faultBits_4B
                    ora       faultBits_4C        ;OR in all bits from faultBits_4C
                    anda      #$F0                ;low nibble unused for all three
                    ora       faultBits_49        ;OR in all bits from faultBits_49
                    ora       faultBits_4A        ;OR in all bits from faultBits_4A
...
; ------------------------------------------------------------------------------
checkForHiSpeed     lda       ignPeriod           ;load MSB of ignition period
...
; if here, RPM > 4185 and TPS > 70%
                    ...
                    lda       bits_201F
                    ora       #$10                ;set bits_201F.4 (to indicate this high-speed condition)
                    sta       bits_201F
                    lda       #$FF
                    sta       specialAdcControl   ;init counter to $FF (used to slow down switch over)
                    bra       startAdcReads

normalSpeed         lda       specialAdcControl   ;branches here if no need for hi-speed ADC mux table
                    beq       useNormalAdcMux     ;if counter is zero, it's time to switch back to normal
                    deca                          ;else, just decrement the delay counter
                    sta       specialAdcControl   ;and store it
                    ldx       #hiRpmAdcMux        ;load address of high-speed table
                    bra       startAdcReads

useNormalAdcMux     lda       bits_201F           ;if here, we are using the normal ADC table
...
startAdcReads       stx       adcMuxTablePtr      ;store pointer in the working (incrementing) location
                    lda       $00,x               ;get the 1st value from the table
                    sta       AdcControlReg1      ;trigger 1st ADC measurement
...
LCB2A               ldx       adcMuxTablePtr      ;load ADC table working (incrementing) pointer
                    ldb       $00,x               ;get next value from table
...
; if here, RPM < 1627
                    lda       acDownCounter       ;down-counter set to 44 dec in A/C routine
                    beq       .LCB4A              ;branch to continue A/C routine if zero

                    deca                          ;otherwise, decrement the down-counter
                    sta       acDownCounter       ;store it
                    bra       .LCB5A              ;and branch to next section

.LCB4A              lda       port2data           ;counter reached zero
                    ldb       $008A
                    bitb      #$08                ;test X008A.3 (A/C 1= OFF, 0 = ON)
                    bne       .LCB56
                    ora       #$04                ;X008A.3 is zero, set P2.2 (A/C compressor)
                    bra       .LCB58
...
; -----------------------------------------------------------
.LCB5A              lda       $0086               ;X0086.3 indicates memory test complete
...
.calcROMChksum      addb      $00,x               ;Loop Start * add add indexed value to B
                    ...
                    stb       romChecksum         ;store the checksum
                    cmpb      #$01                ;compare it with $01
                    beq       .romChksumGood      ;branch if equal

                    lda       faultBits_49
                    ora       #$01                ;set fault code 29 (ROM checksum fail)
                    sta       faultBits_49
                    sta       romChecksumMirror

.romChksumGood      lda       $0086
                    ora       #$08                ;set X0086.3 high (memory test complete)
                    sta       $0086
...
; -----------------------------------------------------------
                    ...
                    #ifdef    NEW_STYLE_MIL_CODE
; this code may be startup flash of warning light
.romTestDone        lda       $C7C2               ;load XC7C2 again (will be $00 or $FF)
                    ...
                    lda       milTestDelay
                    ora       #$80                ;set milTestDelay bit 15
                    sta       milTestDelay
                    lda       port1data
                    ora       #$01                ;EFI warning light OFF
                    sta       port1data
...
; the A register was disturbed by the idleControl
                    ora       #$40                ;subroutine, so this looks like a code bug
                    sta       port1data           ;Port 1.6 high (fuelPumpTimer is zero, fuel pump OFF)
...
; ------------------------------------------------------------------------------
                    ...
                    #ifdef    BUILD_R3365 ;code above branches here if ADC mux bits 6:4 are clear
...
.LCBCE              lda       AdcStsDataHigh      ;read the ADC status/dataHigh register
                    bita      #$40                ;test ADC busy flag
                    bne       .LCBC4              ;branch back if busy

                    anda      #$03                ;save upper 2 bits of 10 bit data
                    sta       $00C8               ;store it in temporary location
                    sta       adcReadingUnused    ;store it here too but this location unused
                    lda       AdcDataLow          ;read low 8 bits
                    sta       $00C9               ;10-bit ADC value is now at X00C8/C9
                    sta       $00BA               ;and 'adcReadingUnused'

                    #ifdef    BUILD_R3365
                    lda       ignPeriod           ;ignition period (MSB only)
                    cmpa      #$0A
                    bcc       .LCBF8
                    cmpa      #$07
                    bcs       .LCBF8
...
.LCBF8              pulb
                    ...
                    lda       $01,x               ;get next ADC control value
                    anda      #$8F                ;force bits 6,5,4 low (ADC doesn't need them)
                    sta       AdcControlReg1      ;write ADC control reg (triggers convert)
...
; -----------------------------------------------------------
                    lda       mpuReInitCounter    ;counter varies between zero and 20
...
.LCC2A              ldd       codeErrorWord       ;never initialized and otherwise unused (to be deleted)
                    ...
                    std       port1ddr
                    sta       port4ddr
                    lda       timerCSR            ;enable Input Capture Interrupt (ICI)
                    ora       #$12                ;set P20 (coil input) to positive edge trigger
                    sta       timerCSR
                    ldd       #$050A
                    std       sciModeControl      ;set serial port to 7812.5 baud
                    lda       #$C0
                    sta       ramControl          ;enable internal RAM , set Standby bit
                    lda       timerCntrlReg1
                    ora       #$A0                ;OCF3 and ICF2
                    sta       timerCntrlReg1
                    lda       #$53                ;enable Input Cap Int 1 and Out Cmpr Int 2
                    sta       timerCntrlReg2      ;low 2 bits are read-only

.LCC52              clra                          ;the 'never used' branch label
.LCC53              sta       mpuReInitCounter    ;store the zero through 20 counter
...
; ---------------------------------------------------------------------
                    ...
                    lda       bits_2047           ;this code executes every 80th time through
...
; -----------------------------------------------------------
                    ...
                    ora       #$01                ;set bits_2047.0
                    sta       bits_2047           ;store bits_2047
...
; ------------------------------
                    ...
                    ldb       $C25C               ;this value is in the range of 6 to 8
...
.LCCF2              cli                           ;clear interrupt mask
                    lda       iacMotorStepCount   ;load stepper motor adjust value
