...
LF416               proc
                    lda       $00E2               ;load bits value
                    bita      #$22                ;test X00E2.5 and X00E2.1
                    beq       Done@@              ;return if both bits are low
...
LF423               proc
                    ldd       throttlePot         ;10-bit value
                    lsrd:2                        ;shift right 2 bits (only top 8 of 10 bits in B)
...
;*******************************************************************************
; Code branches here from below when throttle < about 91% (84% for Griff)
...
;*******************************************************************************
; Code branches here from below when fuel map row index < $30 or RPM < 1838
...
.LF44B              proc
                    anda      #$DF                ;clr X0087.5 (when eng speed is < 3400 RPM)

;*******************************************************************************
; Code gets here for both RPM conditions

.LF44D              proc
                    sta       $0087               ;store value after X0087.5 set or cleared
...
; Eng. speed > 1838 RPM
                    ...
                    lsrd:4                        ;upper 8 bits are now in B accumulator
...
initRAMFromExt      proc
                    ldd       secondaryLambdaR
                    subd      $2060
                    beq       _1@@
                    ldd       #$8000
                    std       secondaryLambdaR

_1@@                ldd       secondaryLambdaL
                    subd      $2064
                    beq       _2@@
                    ldd       #$8000
                    std       secondaryLambdaL

_2@@                ldd       longLambdaTrimR     ;left long-term trim
                    subd      $2062
                    beq       _3@@                ;if different...
                    ldd       #$8000              ;reset to neutral value
                    std       longLambdaTrimR

_3@@                ldd       longLambdaTrimL     ;right long-term trim
                    subd      $2066
                    beq       _4@@                ;if different...
                    ldd       #$8000              ;reset to neutral value
                    std       longLambdaTrimL

_4@@                ldd       hiFuelTemperature   ;checks two 8-bit values
                    subd      $2068               ;1st byte = hiFuelTemperature
                    beq       _5@@                ;2nd byte = faultBits_49
                    clrd
                    std       hiFuelTemperature

_5@@                ldd       faultBits_4A        ;checks two fault bytes
                    subd      $206A               ;(faultBits_4A and faultBits_4B)
                    beq       _6@@                ;and zeros them, if different
                    clrd
                    std       faultBits_4A

_6@@                ldd       faultBits_4C        ;checks two fault bytes
                    subd      $206C               ;(faultBits_4C and faultBits_4D)
                    beq       _7@@                ;and zeros them, if different
                    clrd
                    std       faultBits_4C

_7@@                ldd       faultBits_4E        ;check faultBits_4E and stprMtrSavedValue
                    subd      $206E
                    beq       _8@@                ;if different,
...
_8@@                ldd       throttlePotMinimum  ;16-bit value at X0051/52
                    subd      $2071
                    beq       _9@@                ;if different...
...
_9@@                lda       fuelMapNumberBackup  ;check fuel map number
                    suba      $2070
                    beq       Done@@              ;if different
